"""IPC so Explorer shell verbs reach the already-running DeskTidy instance."""
...
_IPC_WINDOW_CLASS = "DeskTidyShellIpcWnd"
_IPC_WINDOW_TITLE = "DeskTidyShellIpc"
_MAGIC = 0x445401
...
class _IpcBridge(QObject):
    received = pyqtSignal(str, int, int)
...
def _encode_payload(verb: str, x: int, y: int) -> bytes:
    return f"{verb}\t{int(x)}\t{int(y)}".encode("utf-8")


def _decode_payload(data: bytes) -> tuple[str, int, int] | None:
    try:
    ...
    if msg == WM_COPYDATA and _bridge is not None:
        try:
            ...
            if parsed is not None:
                verb, x, y = parsed
                _bridge.received.emit(verb, x, y)
                return 1
        except Exception:
            return 0
...
def start_shell_ipc_host(on_verb: Callable[[str, int, int], None]) -> bool:
    """Create message-only HWND that receives shell-verb COPYDATA messages."""
...
def send_shell_verb_to_running_instance(verb: str, x: int = 0, y: int = 0) -> bool:
    """Send verb to the running DeskTidy IPC window. Returns True if delivered."""
    hwnd = int(user32.FindWindowW(_IPC_WINDOW_CLASS, _IPC_WINDOW_TITLE) or 0)
    if not hwnd:
        return False
    payload = _encode_payload(verb, x, y)
