...
{_op_table([
    ...
    ("账号管理", "云端账号保险库：桌面浮标打开面板，双击复制字段，后台定时同步。"),
...
各按钮的完整用法见对应功能章：录屏、纪要、计算器、待办、分页文件夹。记事本改为 DeskNote 快捷方式，见「记事本」章。
<strong>账号管理</strong>不使用分页浮标，而是独立的桌面圆标，见「账号管理」章。</p>
{_op_table([
    ("录屏 / note / 纪要 / 计算器 / 待办 / 文件夹", "入口在浮标栏；说明分别见左侧同名章节。"),
    ("账号管理", "独立桌面浮标（「账」），不在分页栏上。"),
])}
""",
        "public": f"""
<h2>公共区域</h2>
...
""",
        "vault": f"""
<h2>账号管理</h2>
<p class='purpose'>云端账号保险库：把名称、账号、密码、网址、备注保存在服务器，换设备登录后可同步。
桌面常驻可拖动的「账」圆标（类似豆包浮窗）；单击打开面板，再点一次或点 × 收起。</p>

{_op_table([
    ("主窗口 · 账号管理 · 启用桌面账号浮标", "开启后探测服务器；成功则显示浮标，失败会提示并自动关闭，不阻塞界面。"),
    ("主窗口 · 账号管理 · 后台刷新间隔", "默认定时静默同步（默认 2 小时，可调）；打开面板时不再强制联网。"),
    ("桌面浮标「账」· 单击", "在浮标旁打开 / 收起账号面板；可拖动浮标改变位置。"),
    ("面板 · 标题栏拖动", "可临时移动面板；关闭后再打开会重新贴回浮标旁边。"),
    ("面板 / 设置页 · 登录注册", "手机号或邮箱 + 密码；可设昵称。"),
    ("双击字段", "双击网址 / 账号 / 密码 / 备注即可复制（标题栏有固定提示）。"),
    ("新建 / 编辑 / 删除 / 置顶 / 拖拽排序", "改完后立即刷新列表；搜索时不可排序。"),
    ("导入 / 导出", "设置页可将清单导出为 JSON，或从 JSON 导入到云端。"),
])}
<p class='tip'>服务器不可达时会提示并自动关闭账号管理，避免卡住 DeskTidy。
登录态保存在本地配置目录；密码明文存于云端数据库，请妥善保管账号。</p>
""",
        "pet": f"""
<h2>桌面宠物</h2>
...
<p class='purpose'>桌面分区整理工具：用虚拟钉选分类显示，文件仍留在系统桌面文件夹；
并提供截图、录屏、DeskNote、桌面宠物与云端账号管理等扩展能力。</p>
<ul>
<li>录屏依赖 FFmpeg（安装版已内置）</li>
<li>文件搜索依赖 fd（安装版已内置）</li>
<li>笔记与录屏默认保存在安装目录下的「笔记」「录屏」文件夹</li>
<li>账号管理数据在云端同步；桌面「账」浮标为独立入口</li>
</ul>
"""
